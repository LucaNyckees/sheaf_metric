...
class pipeline:
    ...
    def optim(self, model, fast=False, use_reg=True, alpha=1, lambda_=1, sigma=0.001):
        lr = tf.keras.optimizers.schedules.InverseTimeDecay(
            initial_learning_rate=0.5, decay_steps=10, decay_rate=0.01
        )
        ...
        for epoch in range(50 + 1):
            if epoch > 5:
                diff = tf.abs(
                    tf.norm(optimization["projections"][epoch])
                    - tf.norm(optimization["projections"][epoch - 1])
                )
                ...
                if diff < 0.0005:
                    ...
                    amplitude = alpha * tf.sqrt(
                        wasserstein_distance(dgm1, dgm2, order=2, enable_autodiff=True)
                    )
            ...
            gradients[0] = gradients[0] + np.random.normal(
                loc=0.0, scale=sigma, size=gradients[0].shape
            )
            ...
            for j in range(N):
                if i < j:
                    ...
                    x = np.hstack(
                        (
                            np.array([1]),
                            np.random.rand(n_features - 2) * np.pi,
                    ...
                    m1 = tf.Variable(
                        initial_value=multifilt1, trainable=False, dtype=tf.float32
                    )
                    m2 = tf.Variable(
                        initial_value=multifilt2, trainable=False, dtype=tf.float32
                    )
...
def fast_grid_search(I, J, step=0.01, dim=1):
    if I.shape[-1] != 2:
        print(
            "The fast grid search method should only be applied for the case of bi-filtrations (n_features=2). Here, n_features={}.".format(
                I.shape[-1]
            )
        )
        return None
