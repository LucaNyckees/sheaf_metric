...
def clustering_accuracy(filtration, nb_clusters=2, distance=False):

    if distance :
        
        clustering = AgglomerativeClustering(nb_clusters, affinity="precomputed", linkage='complete')
        clustering.fit(filtration)

    else :

        clustering = AgglomerativeClustering(nb_clusters)
        clustering.fit(filtration)
