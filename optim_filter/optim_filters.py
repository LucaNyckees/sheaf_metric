...
if data_type == "MNIST":
    ...
    if dataset[:2] == "vs":
        ...
        tridxs1, tridxs2 = (
            np.argwhere(X[0][1] == l1).ravel()[::step],
            np.argwhere(X[0][1] == l2).ravel()[::step],
        )
        teidxs1, teidxs2 = (
            np.argwhere(X[1][1] == l1).ravel()[::step],
            np.argwhere(X[1][1] == l2).ravel()[::step],
        )
        IMG = (
            [X[0][0][j] for j in tridxs1]
            + [X[0][0][j] for j in tridxs2]
            + [X[1][0][j] for j in teidxs1]
            + [X[1][0][j] for j in teidxs2]
        )
        LAB = (
            [0 for _ in range(len(tridxs1))]
            + [1 for _ in range(len(tridxs2))]
            + [0 for _ in range(len(teidxs1))]
            + [1 for _ in range(len(teidxs2))]
        )
        ...
        IMG = [X[0][0][j] for j in range(len(X[0][0]))][::step] + [
            X[1][0][j] for j in range(len(X[1][0]))
        ][::step]
        LAB = [X[0][1][j] for j in range(len(X[0][0]))][::step] + [
            X[1][1][j] for j in range(len(X[1][0]))
        ][::step]
    ...
    for pdi in range(num_filts):
        ...
        for j, img in enumerate(IMG):
            ...
            if j == -1:
                vm, vM = min(list(I[I != np.inf].flatten())), max(
                    list(I[I != np.inf].flatten())
                )
                plt.figure()
    ...
    if use_spektral:
        ...
        for graph in list_graph_names:
            if graph[-4:] == ".mat":
                ...
                eigenvals[: min(pad_size, num_vertices)] = np.flipud(egvals)[
                    : min(pad_size, num_vertices)
                ]
                eigenvectors[:, : min(pad_size, num_vertices)] = np.fliplr(egvectors)[
                    :, : min(pad_size, num_vertices)
                ]
...
for f in range(num_filts):
    ...
    if dgms[np.argwhere(dgms[:, 1] != np.inf).ravel()].shape[0] == 0:
        DGMb[f] = [np.array([[0.0, 0.0], [1.0, 1.0]]) for _ in range(num_diags)]
            ...
            FeatureUnion(
                [
                    (
                        ...
                        Pipeline(
                            [
                                ("Selector", FiltrationSelector()),
                                (
                                    "Separator",
                                    sktda.DiagramSelector(
                                        limit=np.inf, point_type="finite"
                                    ),
                                ),
                                ("TDA", sktda.Landscape()),
                            ]
...
param = {
    "Feats__Pipe0__Selector__use": True,
...
for nf in range(num_filts - 1):
    newparam = {
        "Feats__Pipe" + str(nf + 1) + "__Selector__use": True,
...
if data_type == "MNIST":
    train_imgs, test_imgs = np.vstack(
        [IMG[i].flatten()[np.newaxis, :] for i in train_idxs]
    ), np.vstack([IMG[i].flatten()[np.newaxis, :] for i in test_idxs])
    ...
    train_evas, test_evas = np.vstack(
        [EVA[i].flatten()[np.newaxis, :] for i in train_idxs]
    ), np.vstack([EVA[i].flatten()[np.newaxis, :] for i in test_idxs])
...
if use_sliced:
    thetas = tf.Variable(
        initial_value=np.array(thetainit, dtype=np.float32), trainable=False
    )
...
lr = tf.keras.optimizers.schedules.ExponentialDecay(
    initial_learning_rate, decay_steps=1e5, decay_rate=0.99, staircase=True
)
...
for epoch in range(num_epochs + 1):
    ...
    while npts < total_n:
        ...
        with tf.GradientTape() as tape:
            ...
            for nf in range(num_filts):
                if data_type == "MNIST":
                    ...
                    for i in batch:
                        if use_spektral:
                            ...
                            func = tf.math.reduce_sum(
                                tf.math.multiply(
                                    C[nf, :], tf.constant(EVE[i], dtype=tf.float32)
                                ),
                                axis=1,
                            )

                        dgm = SimplexTreeModel(
                            F=func,
                            stbase=path + graph_names[i] + "_st.txt",
                            dim=hdim,
                            card=hcard,
                        ).call()
                        dgms.append(dgm)

                proj_dgms = tf.linalg.matmul(
                    tf.concat(dgms, axis=0), 0.5 * tf.ones([2, 2], tf.float32)
                )
                dgms_big = tf.concat(
                    [
                        tf.reshape(
                            tf.concat(
                                [
                                    dgm,
                                    proj_dgms[: hcard * idg],
                                    proj_dgms[hcard * (idg + 1) :],
                                ],
                                axis=0,
                            ),
                            [-1, 2, 1, 1],
                        )
                        for idg, dgm in enumerate(dgms)
                    ],
                    axis=2,
                )
                ...
                vecs = tf.concat(
                    [
                        tf.reshape(cosines, [1, 1, 1, -1]),
                        tf.reshape(sines, [1, 1, 1, -1]),
                    ],
                    axis=1,
                )
                theta_projs = tf.sort(
                    tf.math.reduce_sum(tf.math.multiply(dgms_big, vecs), axis=1), axis=0
                )
                t1 = tf.reshape(
                    theta_projs, [hcard * len(batch), -1, 1, numdir]
                )  # [hcard*batch_size,-1,1,numdir])
                t2 = tf.reshape(
                    theta_projs, [hcard * len(batch), 1, -1, numdir]
                )  # [hcard*batch_size,1,-1,numdir])
                dists.append(
                    tf.math.reduce_mean(
                        tf.math.reduce_sum(tf.math.abs(t1 - t2), axis=0), axis=2
                    )
                )
    ...
    if epoch % 10 == 0:
        ...
        if data_type == "MNIST":
            ...
            for nf in range(num_filts):
                ...
                for i, img in enumerate(IMG):
                    ...
                    for i in range(len(inds)):
                        val = (
                            np.cos(final_coeff[nf]) * inds[i, 0]
                            + np.sin(final_coeff[nf]) * inds[i, 1]
                        )
                        I[inds[i, 0], inds[i, 1]] = val
        ...
        for f in range(num_filts):
            ...
            if dgms[np.argwhere(dgms[:, 1] != np.inf).ravel()].shape[0] == 0:
                DGM[f] = [np.array([[0.0, 0.0], [1.0, 1.0]]) for _ in range(num_diags)]
        ...
        DGM = [
            [
                DGM[f][i]
                if DGM[f][i].shape[0] != 0
                else np.array([[0.0, 0.0], [1.0, 1.0]])
                for f in range(num_filts)
            ]
            for i in range(num_diags)
        ]
        train_dgms, test_dgms = [DGM[i] for i in train_idxs], [
            DGM[i] for i in test_idxs
        ]
