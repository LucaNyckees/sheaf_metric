...
mnist_ds = (
    []
)  # ['all'] + ['vs' + str(i) + str(j) for i in range(0,9) for j in range(i+1,10)]
graphs_ds = [
    "PROTEINS",
    "MUTAG",
    "COX2",
        ...
        if not folds:
            ...
            for f in faccs:
                ...
                try:
                    ...
                    AS.append(
                        np.array(
                            [[float(a) for a in l[:-1].split(" ")[:-1]] for l in L]
                        )
                    )
            ...
            if ds in graphs_ds:
                if te[ep] - teb[ep] > 0.0:
                    ftable.write(
                        ...
                        + "{:.1f}".format(tebb[ep])
                        + " $\\pm$ "
                        + "{:.2f}".format(std_tebb[ep])
                        + " & "
                        + "{:.1f}".format(teb[ep])
                        + " $\\pm$ "
                        + "{:.2f}".format(std_teb[ep])
                        + " & "
                        + "{:.1f}".format(te[ep])
                        + " $\\pm$ "
                        + "{:.2f}".format(std_te[ep])
                        + " & \\bf{"
                        + "+{:.1f}".format(te[ep] - teb[ep])
                        + "} \\\\ \n"
                    )
                else:
                    ftable.write(
